...
import { getShiki, initHighlighter, loadLanguage, loadTheme } from './highlighter';
...
export const ShikiPlugin = (
    ...
    const shikiPlugin: Plugin<any> = new Plugin({
        ...
        state: {
            init: (_, { doc }) => getDecorations({ doc, name, defaultLanguage, defaultTheme }),
            apply: (transaction, decorationSet, oldState, newState) => {
                ...
                if (transaction.getMeta('shikiPluginForceDecoration') || didChangeSomeCodeBlock)
                    return getDecorations({ doc: transaction.doc, name, defaultLanguage, defaultTheme });
